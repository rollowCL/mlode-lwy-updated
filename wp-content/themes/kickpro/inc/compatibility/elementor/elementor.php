...
class Awaiken_Elementor { 
	...
	public static function elementor_free_menu() {
		...
		$parent_slug = 'edit.php?post_type=elementor_library';
		remove_submenu_page($parent_slug, 'elementor-app'); 
		remove_submenu_page($parent_slug, 'popup_templates'); 
		remove_submenu_page($parent_slug, 'go_cloud_hosting_plans'); 
	}
	...
	public static function register_new_widgets( $widgets_manager ) {
		...
		require_once KICKPRO_THEME_DIR . '/inc/compatibility/elementor/widgets/site-logo.php';
		require_once KICKPRO_THEME_DIR . '/inc/compatibility/elementor/widgets/template.php';
		require_once KICKPRO_THEME_DIR . '/inc/compatibility/elementor/widgets/breadcrumb.php';

		$widgets_manager->register( new \kickpro_widget_site_Logo() );
		$widgets_manager->register( new \kickpro_widget_template() );
		$widgets_manager->register( new \kickpro_widget_breadcrumb() );

	}
	
	public static function elementor_editor_styles() {
		wp_enqueue_style( 'kickpro-elementor-editor', KICKPRO_THEME_URL . '/assets/css/elementor-editor.css', array(), KICKPRO_THEME_VERSION );
		...
		if ( !defined( 'ELEMENTOR_PRO_VERSION' ) ) {
			...
			wp_add_inline_style( 'kickpro-elementor-editor', $custom_css );
		}
	}
	
	public static function elementor_preview_styles() {
		wp_enqueue_style( 'kickpro-elementor-preview-style', KICKPRO_THEME_URL . '/assets/css/elementor-preview.css', array(), KICKPRO_THEME_VERSION );
	}
	
	public function update_default_elementor_kit() {

		add_option( 'default_kickpro_kit', 0 );
		if ( get_option( 'default_kickpro_kit' ) === '0' ) {
			...
			update_option( 'default_kickpro_kit', 1 );
		}
	}
