 ...
 * @subpackage Example
 * @version    2.6.1 for parent theme Kickpro for publication on ThemeForest
...
function kickpro_json_error_license_invalid() {
    echo '<div class="notice notice-error is-dismissible">
        <p>' . esc_html__( 'Invalid license', 'kickpro' ) . '</p>
    </div>';
}

function kickpro_json_error_notice() {
    echo '<div class="notice notice-error is-dismissible">
        <p>' . esc_html__( 'Unable to connect to Awaiken server. Failed to retrieve valid JSON data.', 'kickpro' ) . '</p>
    </div>';
}

function kickpro_http_error_notice() {
    echo '<div class="notice notice-error is-dismissible">
		<p>' . esc_html__( 'Unable to connect to Awaiken server. Failed to fetch data from the server.', 'kickpro' ) . '</p>
    </div>';
}

function kickpro_get_plugin_versions() {
    ...
    if ( !is_wp_error( $response ) && wp_remote_retrieve_response_code( $response ) === 200 ) {
        ...
        if ( $data !== null && isset( $data['data'] ) ) {
            ...
            add_action('admin_notices', 'kickpro_json_error_license_invalid');
            return null;
		} 
		else {
            // Show an admin notice if JSON structure is unexpected
            add_action('admin_notices', 'kickpro_json_error_notice');
            return null;
        }
    } else {
        // Show an admin notice if the HTTP request fails
        add_action('admin_notices', 'kickpro_http_error_notice');
        return null;
    }
}
require_once KICKPRO_THEME_DIR . '/inc/class-tgm-plugin-activation.php';

add_action( 'tgmpa_register', 'kickpro_register_required_plugins' );
 ...
 */
function kickpro_register_required_plugins() {
	/*
	 * Array of plugin arrays. Required keys are name and slug.
	 * If the source is NOT from the .org repo, then source is also required.
	 */
	 
	$plugin_versions = kickpro_get_plugin_versions();
	...
	$plugins = array(
		...
		array(
			'name'              => esc_html__('Kickpro Theme Addons', 'kickpro'), 
			'slug'              => 'kickpro-theme-addons',
			'source'            => get_template_directory() . '/assets/plugins/kickpro-theme-addons.zip', 
			'version'			=> '1.0.1',
			'required'          => true, 
		),
		array(
			'name'      		=> esc_html__('Contact Form 7', 'kickpro'),
			'slug'      		=> 'contact-form-7',
			'required'  		=> true,
		),
		array(
			'name'      		=>  esc_html__('Custom Fonts', 'kickpro'),
			'slug'      		=> 'custom-fonts',
			'required'  		=> true,
		)
	);
	...
	if( get_option( 'kickpro_demo_imported' ) !== '1' ) { 
		$plugins[] = array(
			'name'     			=> esc_html__('One Click Demo Import', 'kickpro') ,
	 ...
	 */
	$config = array(
		'id'           => 'kickpro',                 // Unique ID for hashing notices for multiple instances of TGMPA.
