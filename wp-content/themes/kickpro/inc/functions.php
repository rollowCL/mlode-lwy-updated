...
*	Register action
*/
add_action( 'kickpro_action_get_breadcrumb', 'kickpro_get_breadcrumb' );
add_action( 'kickpro_action_social_sharing', 'kickpro_get_social_sharing_icons' );

function kickpro_get_social_sharing_icons() {
	$sharing_links = kickpro_generate_social_share_links();
	if($sharing_links) {
 ...
 * Add class to body
*/
add_filter( 'body_class', 'kickpro_body_class' );
function kickpro_body_class( $classes ) {
	global $KICKPRO_STORAGE;
	
	$kickpro_active_demo =  get_option( 'kickpro_active_demo' );
	
    if( get_theme_mod( 'magic_cursor', $KICKPRO_STORAGE['magic_cursor'] ) ) { 
        $classes[] = 'tt-magic-cursor';
    }

	if( !get_theme_mod( 'show_small_heading_icon', $KICKPRO_STORAGE['show_small_heading_icon'] ) ) { 
        $classes[] = 'kickpro-hide-small-icon';
    }

	if( get_theme_mod( 'custom_fancy_scrollbar', $KICKPRO_STORAGE['custom_fancy_scrollbar'] ) ) { 
	...
	if( isset($_GET['elementskit_template']) && !isset($_GET['elementor-preview']) && ( in_array($_GET['elementskit_template'], array( 'header', 'header-layout-2', 'header-layout-3', 'header-layout-4') ) ) ) {
		$classes[] = 'kickpro-header-preview';
	}
	
	if($kickpro_active_demo) {
		$classes[] = $kickpro_active_demo;
	}
	
    return $classes;
}

add_action( 'wp_body_open', 'kickpro_wp_body_open' );

function kickpro_wp_body_open() {
	global $KICKPRO_STORAGE;

	if( !is_admin() && get_theme_mod( 'show_preloader', $KICKPRO_STORAGE['show_preloader'] ) ) { 
	
	$icon = get_theme_mod( 'preloader_icon', $KICKPRO_STORAGE['preloader_icon'] );
	$preloader_icon = KICKPRO_THEME_URL.'/assets/images/loader.svg';
	if ( !empty($icon) ) { 
	...
	<div class="theme-preloader">
		<div class="loading-container">
			...
			<div id="loading-icon"><img src="<?php echo esc_url($preloader_icon); ?>" alt="<?php echo esc_attr__( 'Preloader', 'kickpro' ); ?>"></div>
		</div>
	</div>
	<?php 
	} 
...
function kickpro_add_additional_class_on_li($classes, $item, $args) {
	if (property_exists($args, 'li_class')) {
        $classes[] = $args->li_class;
    }
    return $classes;
}
add_filter('nav_menu_css_class', 'kickpro_add_additional_class_on_li', 1, 3);

/*
* Add class to header menu li a tag
*/

function kickpro_add_additional_class_to_a( $atts, $item, $args ) {
  if (property_exists($args, 'a_tag_class')) {
    $atts['class'] = $args->a_tag_class;
  }
  return $atts;
}
add_filter( 'nav_menu_link_attributes', 'kickpro_add_additional_class_to_a', 1, 3 );
...
function kickpro_fallback( $args ) {
	...
	$fallback_output .= '<li class="nav-item"><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" class="nav-link" title="' . esc_attr__( 'Add a menu', 'kickpro' ) . '">' . esc_html__( 'Add a menu', 'kickpro' ) . '</a></li>';
	$fallback_output .= '</ul>';
...
function kickpro_license_valid() {
...
if ( ! function_exists( 'kickpro_get_archive_title' ) ) {
	function kickpro_get_archive_title() {

		if ( is_front_page() ) {
			$title = esc_html__( 'Home', 'kickpro' );
		} elseif ( is_home() ) {
			$title = get_theme_mod( 'blog_page_title' );
			if(empty($title)) {
				$title = esc_html__( 'Blog', 'kickpro' );
			}
		} elseif ( is_author() ) {
			$curauth = ( get_query_var( 'author_name' ) ) ? get_user_by( 'slug', get_query_var( 'author_name' ) ) : get_userdata( get_query_var( 'author' ) );
			// Translators: Add the author's name to the title
			$title = sprintf( esc_html__( 'Author: %s', 'kickpro' ), $curauth->display_name );
		} elseif ( is_404() ) {
			$title = esc_html__( 'URL not found', 'kickpro' );
		} elseif ( is_search() ) {
			// Translators: Add the author's name to the title
			$title = sprintf( esc_html__( 'Search: %s', 'kickpro' ), get_search_query() );
		} elseif ( is_day() ) {
			// Translators: Add the queried date to the title
			$title = sprintf( esc_html__( 'Daily Archives: %s', 'kickpro' ), get_the_date() );
		} elseif ( is_month() ) {
			// Translators: Add the queried month to the title
			$title = sprintf( esc_html__( 'Monthly Archives: %s', 'kickpro' ), get_the_date( 'F Y' ) );
		} elseif ( is_year() ) {
			// Translators: Add the queried year to the title
			$title = sprintf( esc_html__( 'Yearly Archives: %s', 'kickpro' ), get_the_date( 'Y' ) );
		} elseif ( is_category() ) {
			$title = single_cat_title( '', false );
		} elseif ( is_tag() ) {
			// Translators: Add the tag's name to the title
			$title = sprintf( esc_html__( 'Tag: %s', 'kickpro' ), single_tag_title( '', false ) );
		} elseif ( is_tax() ) {
		...
		elseif ( is_post_type_archive() ) {
			...
			$title = sprintf( esc_html__( 'Attachment: %s', 'kickpro' ), get_the_title() );
		} elseif ( is_single() || is_page() ) {
			$title = get_the_title();
		} else {
			$title = get_the_title();
		}
		return apply_filters( 'kickpro_filter_get_archive_title', $title );
	}
}
 ...
 */
if ( ! function_exists( 'kickpro_social_icons_list' ) ) {
	function kickpro_social_icons_list() {
		...
		return apply_filters( 'kickpro_social_icons', $social_icons );
	}
}
...
if ( ! function_exists( 'kickpro_get_social_media' ) ) {
	function kickpro_get_social_media() {
		global $KICKPRO_STORAGE;
		$output = array();
		$social_icons = kickpro_social_icons_list();
		$social_urls = explode( ',', get_theme_mod( 'social_urls', $KICKPRO_STORAGE['social_urls'] ) );
		...
		if ( !empty( $output ) ) {
			$output = apply_filters( 'kickpro_social_profile_list', $output );
 ...
 * Social share links
 */
if ( ! function_exists( 'kickpro_generate_social_share_links' ) ) {
	function kickpro_generate_social_share_links() {
		
		global $KICKPRO_STORAGE;
		$social_sharing_links = explode( ',', get_theme_mod( 'social_sharing', $KICKPRO_STORAGE['social_sharing'] ) );
...
if ( ! function_exists( 'kickpro_custom_excerpt_length' ) ) {
	function kickpro_custom_excerpt_length( $length ) {
		return apply_filters( 'kickpro_excerpt_length', 12 );
	}
}

add_filter( 'excerpt_length', 'kickpro_custom_excerpt_length', 999 );

if ( ! function_exists( 'kickpro_render_icon_tag' ) ) {
    function kickpro_render_icon_tag( $file ) {
        return '<i class="fa-solid fa-arrow-right"></i>';
    }
}

if ( ! function_exists( 'kickpro_render_svg' ) ) {
    function kickpro_render_svg( $file ) {
        $valid_svg = false;
        ...
        if ( false === $valid_svg ) {
            $valid_svg = kickpro_render_icon_tag( $file );
        }
